"""
Release SubSection 프롬프트

Spec 6.5.2 참조: Release 섹션에서 Item 추출
Spec 3.3.1~3.3.4 참조: Item 필드 정의
"""

RELEASE_PROMPT = """You are an expert Item extractor for 3GPP RAN1 Release sections.

# Role
Extract Items from the Release section content. Each Item represents a complete discussion flow.

# Key Characteristics of Release Items
- Agreement-centric: New spec writing, direction consensus
- Working Assumption: Tentative assumptions not yet confirmed
- FFS (For Further Study): Items requiring additional investigation
- Noted: Individual TDoc may be "noted"
- Focus on new functionality definition

# Difference from Maintenance
- Maintenance: Modify existing specs (CR-centric)
- Release: Write new specs (Agreement-centric)
- CR ratio is lower in Release sections
...
## Topic 동일성 판단
- Summary 제목: "FL Summary #N on {{Topic}}" - {{Topic}} 부분이 동일하면 같은 Topic
- TDoc 연속성: 동일 TDoc 또는 revision 관계
- 기술 주제: 같은 파라미터, 같은 기능에 대한 논의

## 병합 규칙
- "FL Summary #1 on Topic A" → "FL Summary #2 on Topic A" = **1개 Item** (병합!)
- Monday: Deferred → Thursday: Agreed = **status: Agreed** (최종값만 기록)
- 여러 세션 논의 = comeback: true, first_discussed + concluded 기록

## 올바른 예시 ✓
Topic "beam management" 논의가 여러 세션에 걸쳐 진행됨:
```yaml
- id: "RAN1_120_9.2.1_001"
  topic:
    summary: "UE-initiated beam reporting"
  input:
...
```yaml
- id: "RAN1_120_9.2.1_001"
  status: Deferred  # Monday ← 세션별로 분리하면 안됨!
- id: "RAN1_120_9.2.1_002"
  status: Agreed  # Thursday ← 중복!
```
...
**형식 1 - 탭 구분 (가장 흔함)**:
```
R1-2500055	Discussion of the adaptation of signal	FUTUREWEI
R1-2500079	On common channel adaptation for eNES	Huawei, HiSilicon
```

**형식 2 - 링크 형식**:
```
[R1-2500055](link) Discussion of the adaptation... FUTUREWEI
```
...
- `**R1-2501451****	Summary#1 of AI 9.5.3 for R19 NES****	Moderator (Ericsson)**` → R1-2501451
- `**R1-2501452****	Summary#2 of AI 9.5.3 for R19 NES****	Moderator (Ericsson)**` → R1-2501452
- `**R1-2501453****	Summary#3 of AI 9.5.3 for R19 NES****	Moderator (Ericsson)**` → R1-2501453
- **반드시** 모든 Summary를 배열로 수집: ["R1-2501451", "R1-2501452", "R1-2501453"]
- Summary TDoc 식별: 제목에 "Summary", "FL summary" 포함

```yaml
input:
  moderator_summary: ["R1-2501451", "R1-2501452"]  # 해당 Item과 관련된 Summary들
  discussion_tdocs:
    - tdoc_id: "R1-2500055"
      title: "Discussion of the adaptation of signal/channel"
      source: "FUTUREWEI"
      tdoc_type: discussion
    - tdoc_id: "R1-2500079"
      title: "On common channel adaptation for eNES"
...
[
  {{
    ...
    "context": {{
      ...
      "section_id": "9",
      "leaf_id": "{leaf_id}",
      "leaf_title": "Example topic",
      "section_type": "Release"
    }},
    "topic": {{"summary": "UE-initiated beam reporting for MIMO"}},
    "input": {{
      "moderator_summary": ["R1-2501410", "R1-2501520"],
      "discussion_tdocs": [
        {{
    ...
    "resolution": {{
      ...
      "content": [
        {{
          "type": "agreement",
          "text": "RAN1 agrees on the UE-initiated beam reporting procedure",
